...
function mrp:SetCurrentProfile( profile, isauto )
    ...
    for field, value in pairs( mrpSaved.Profiles.Default ) do
        if tContains(mspValidFields, field) then
            if(field == "MU") then -- All fields must be a string, but it comes through as a number.
                msp.my[ field ] = tostring(value)
            else
                msp.my[ field ] = value
            end
        end
...
function mrp:SaveField( field, newtext )
    ...
    if newtext then
        if(field ~= "MU") then -- Music is stored as a number
            newtext = strtrim( newtext )
        end
        if(field == "PE") then
            newtext = newtext:gsub("|n", ""):gsub("|c%x%x%x%x%x%x%x%x", ""):gsub("|r", ""):gsub("|H.-|h(.-)|h", "%1"):gsub("|K.-|k.-|k", ""):trim()
        elseif(field ~= "MU") then -- MU is a number so trying to gsub on it breaks things.
		...
		if(field == "NA" and newtext == "") then -- Don't allow players to set blank names, it breaks stuff.
			newtext = UnitName("player");
			mrp:Print("You can't set a blank name.")
		end
    end
