...
local BROWSER_DEFAULT_PORTRAIT = [[Interface\Icons\trade_archaeology_draenei_tome]];
local BROWSER_DEFAULT_WIDTH = 338; -- 450
local BROWSER_DEFAULT_HEIGHT = 424; -- 475

if mrp:IsClassicClient() then
    BROWSER_DEFAULT_PORTRAIT = [[Interface\Icons\INV_Misc_Book_01]];
end

-- TODO: Fix these global leaks.
local bfa, bfb, bfc;
local glances;
local personalityTraits;

mrp.DEFAULT_TRAITS_MAPPING = { -- LI = Left Icon, RI = Right Icon, LT = Left Text, RT = Right Text
    [1] = {
        ["LI"] = mrp:IsMainlineClient() and "Ability_Rogue_WrongfullyAccused" or "Spell_Shadow_UnholyFrenzy",
        ["RI"] = mrp:IsMainlineClient() and "Ability_Paladin_SanctifiedWrath" or "Spell_Holy_RighteousFury",
        ["LT"] = L["lefttrait1"],
        ["RT"] = L["righttrait1"]
    },
    [2] = {
    ...
    [4] = {
        ["LI"] = "INV_Misc_Gift_02",
        ["RI"] = mrp:IsMainlineClient() and "INV_Misc_Coin_02" or "INV_Ingot_03",
        ["LT"] = L["lefttrait4"],
        ["RT"] = L["righttrait4"]
    },
    [5] = {
        ["LI"] = mrp:IsMainlineClient() and "INV_Misc_Toy_07" or "Spell_Holy_AuraOfLight",
    ...
    [6] = {
        ...
        ["RI"] = mrp:IsMainlineClient() and "Ability_Warrior_Trauma" or "Ability_Rogue_Eviscerate",
        ["LT"] = L["lefttrait6"],
        ["RT"] = L["righttrait6"]
    },
    [7] = {
        ["LI"] = mrp:IsMainlineClient() and "Spell_Holy_HolyGuidance" or "Spell_Holy_PowerInfusion",
        ["RI"] = "INV_Gizmo_02",
        ["LT"] = L["lefttrait7"],
        ["RT"] = L["righttrait7"]
    },
    [8] = {
        ["LI"] = mrp:IsMainlineClient() and "Ability_Rogue_HonorAmongstThieves" or "Ability_Rogue_DualWeild",
        ["RI"] = mrp:IsMainlineClient() and "INV_Misc_GroupNeedMore" or "ACHIEVEMENT_GUILDPERK_HAVEGROUP WILLTRAVEL",
        ["LT"] = L["lefttrait8"],
        ["RT"] = L["righttrait8"]
    },
    [9] = {
        ["LI"] = mrp:IsMainlineClient() and "Spell_Shadow_Brainwash" or "INV_Misc_PocketWatch_01",
        ["RI"] = mrp:IsMainlineClient() and "Achievement_BG_CaptureFlag_EOS" or "SPELL_FIRE_INCINERATE",
        ["LT"] = L["lefttrait9"],
        ["RT"] = L["righttrait9"]
    },
    [10] = {
        ["LI"] = mrp:IsMainlineClient() and "INV_Misc_Food_PineNut" or "INV_Misc_Coin_05",
        ["RI"] = mrp:IsMainlineClient() and "INV_Misc_Food_99" or "INV_Misc_Coin_02",
        ["LT"] = L["lefttrait10"],
        ["RT"] = L["righttrait10"]
    },
    [11] = {
        ["LI"] = mrp:IsMainlineClient() and "Ability_Paladin_BeaconofLight" or "Ability_Warrior_BattleShout",
...
local function mrp_BrowseFrameSizeUpdate( bf, width, height )
...
function mrp:CreateBrowseFrame()
    if not MyRolePlayBrowseFrame then
        ...
        if mrpSaved.Positions.Browser then
            ...
            bf:SetSize( mrpSaved.Positions.Browser[4] or BROWSER_DEFAULT_WIDTH, mrpSaved.Positions.Browser[5] or BROWSER_DEFAULT_HEIGHT )
        ...
        SetPortraitToTexture( "MyRolePlayBrowseFramePortrait", BROWSER_DEFAULT_PORTRAIT )
        ...
        bf:SetMinResize( BROWSER_DEFAULT_WIDTH, BROWSER_DEFAULT_HEIGHT ) -- from UIPanelTemplates.xml > ButtonFrameTemplate > PortraitFrameTemplate (was 338, 424)

        -- Private notes button
        bf.npb = CreateFrame( "Button", "MyRolePlayBrowseFrame_NotesButton", bf, "UIPanelButtonTemplate" )
        ...
        bf.npb:SetScript("OnClick", function (self)
            ...
            MyRolePlayNotesFrameEditBox:SetFocus();
        end )

        -- Play music button
        bf.pmb = CreateFrame( "Button", "MyRolePlayBrowseFrame_PlayMusicButton", bf, "UIPanelButtonTemplate" )
        ...
        mrp.CreateBrowseFrame = nop
        mrp.CreateBFpfield = nop
    end
end
    ...
    if(statusType == "MESSAGE") then
        if(MyRolePlayBrowseFrame:IsShown() == true and name == lastViewedPlayer) then
            if(msgID == msgTotal) then
                MyRolePlayBrowseFrame.load:SetText(string.format("%s (%s)", L["browser_loading_complete"], Ambiguate(lastViewedPlayer or "Unknown", "short")))
            else
        ...
        MyRolePlayBrowseFrame.load:SetText(string.format("%s (%s)", L["browser_loading_error"], Ambiguate(lastViewedPlayer or "Unknown", "short")))
    end
end
...
function mrp:UpdateBrowseFrame( player )
    ...
    MyRolePlayBrowseFrameTitleText:SetText( mrp.DisplayBrowser.NA( emptynil(mrp:IncreaseColourContrast(f.NA)) or player ) )
    ...
    local musicFileID = tonumber(f.MU)
    if not musicFileID and type(f.MU) == "string" then -- Convert old path to fileID if old versions send us that.
        musicFileID = LibRPMedia:GetMusicFileByName(f.MU);
    end
    ...
    if(musicFileID) then
        ...
        MyRolePlayBrowseFrame_PlayMusicButton:SetScript( "OnClick", function(self)
            ...
            previousSoundHandle = select(2, PlaySoundFile(LibRPMedia:GetNativeMusicFile(musicFileID), "Master"))
        end )
        ...
        MyRolePlayBrowseFrame_PlayMusicButton:SetScript( "OnEnter", function(self)
            ...
            GameTooltip:AddLine( musicName, 1.0, 1.0, 1.0, false )
            GameTooltip:Show()
        end )
        bf.pmb:SetScript( "OnLeave", GameTooltip_Hide )
    ...
    bf.load:SetText( L["browser_loading_nonewdata"] .. " (" .. Ambiguate(player, "short") .. ")")
    ...
    bfa.fields.RC.t:SetText( emptynil( mrp:IncreaseColourContrast(mrp.DisplayBrowser.RC( f.RC ))) or L[ mrp.DisplayBrowser.GC( f.GC ) ] ) -- RC
    ...
    local t = mrp.DisplayBrowser.CU( f.CU )
...
function mrp:BrowseFrameReset()
    ...
    MyRolePlayBrowseFrame:SetSize( BROWSER_DEFAULT_WIDTH, BROWSER_DEFAULT_HEIGHT )
    mrp_BrowseFrameSizeUpdate( MyRolePlayBrowseFrame, MyRolePlayBrowseFrame:GetWidth(), MyRolePlayBrowseFrame:GetHeight() )
end
