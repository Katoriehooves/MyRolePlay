...
mrp.Commands = {
    ["version"] = function( )
        mrp:Print( L["%s by |cffffc1fbEtarna Moonshyne|r and |cffffc1fbKatorie|r."], mrp.VerInfo )
    end,
    ["help"] = function ( )
    ...
    ["profile"] = function( para, paras, input )
        local _, profilename = strsplit(" ", strtrim(input), 2)
    ...
    ["show"] = function( para, paras, input )
        ...
        elseif para ~= "" then
            local _, name = strsplit(" ", strtrim(input), 2)
            local realm;
            name = strtrim( name )
            if(name:match("%-")) then
                realm = name:match("%-(.+)")
    ...
    ["currently"] = function( para, paras, input )
        local _, currently = strsplit(" ", strtrim(input), 2)
