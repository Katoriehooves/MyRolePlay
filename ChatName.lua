...
function mrp:RefreshRPChats()
    ...
    if(mrpSaved.Options.RPChatGuild == true) then
        RPEVENTS["CHAT_MSG_GUILD"] = true;
        RPEVENTS["CHAT_MSG_OFFICER"] = true;
    end
end
...
local function mrp_GetColoredName( event, message, sender, language, arg4, arg5, arg6, arg7, arg8, arg9, arg10, lineid, guid)
    if RPEVENTS[ event ] and sender and sender ~= "" and sender ~= UNKNOWN and msp.char[ sender ].supported and msp.char[ sender ].field.NA and mrp.DisplayChat.NA( msp.char[ sender ].field.NA ) ~= "" and mrp.DisplayChat.NA( msp.char[ sender ].field.NA ) ~= sender then
        if ChatTypeInfo[ strsub( event, 10 ) or "" ] and guid ~= "" then
            if(mrpSaved.Options.ShowIconsInChat == true and msp.char[sender].field.IC and msp.char[sender].field.IC ~= "") then -- Toggle for showing icons beside player name in chat.
                return format( "%s%s%s|r", mrp:WrapIconFilename(msp.char[sender].field.IC, 15, 15), RAID_CLASS_COLORS_CODE[ ( select( 2, GetPlayerInfoByGUID( guid ) ) ) ], mrp:IncreaseColourContrast(mrp.DisplayChat.NA( msp.char[ sender ].field.NA )) )
            else
                return format( "%s%s|r", RAID_CLASS_COLORS_CODE[ ( select( 2, GetPlayerInfoByGUID( guid ) ) ) ], mrp:IncreaseColourContrast(mrp.DisplayChat.NA( msp.char[ sender ].field.NA )) )
            end
        end
    else
