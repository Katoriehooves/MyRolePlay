...
function mrp:FormChanged( istransform )
    ...
    if not istransform and mrp.oldform == form then
        mrp:DebugSpam( "form:%d->%d (no change)", (mrp.oldform or 0), (form or 0) )
        return
    end
...
function mrp:HookFormChange()
    if select( 2, UnitRace("player") ) == "Worgen" or
        ...
        mrp.oldtransformid = mrpSaved.HumanForm
        df:RegisterEvent( "UPDATE_SHAPESHIFT_FORM" )
        if select( 2, UnitRace("player") ) == "Worgen" then
            df:RegisterEvent( "COMBAT_LOG_EVENT_UNFILTERED" )
            df:RegisterEvent( "PLAYER_REGEN_DISABLED" )
        end
    end
end
...
function mrp:UnhookFormChange()
    if mrp.formchangehooked then
        df:UnregisterEvent( "UPDATE_SHAPESHIFT_FORM" )
        df:UnregisterEvent( "COMBAT_LOG_EVENT_UNFILTERED" )
        df:UnregisterEvent( "PLAYER_REGEN_DISABLED" )
