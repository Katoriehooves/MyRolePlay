...
function mrp:AddMRPTab()
    if not mrp.Tabname then
        ...
        if(mrp:IsMainlineClient() or select( 2, UnitClass("player") ) ~= "HUNTER" or select( 2, UnitClass("player") ) ~= "WARLOCK") then
            -- In vanilla we need a shorter tab for hunters due to space issues.
            tab:SetText( L["tabtitle"] )
        else
            tab:SetText( L["tabtitleshort"] )
        end

        tab:SetPoint( "LEFT", "CharacterFrameTab"..CharacterFrame.numTabs, "RIGHT", -15, 0 )
        tab:SetScript( "OnEnter", function(self)
            GameTooltip:SetOwner( self, "ANCHOR_RIGHT" )
            GameTooltip:SetText( L["editor_mrptab_tt"], 1.0, 1.0, 1.0 )
        end )
        ...
        if mrp:IsMainlineClient() then
            -- Add our subframe to the list of frames (Taints CHARACTERFRAME_SUBFRAMES as of 8.2)
            -- tinsert( CHARACTERFRAME_SUBFRAMES, "MyRolePlayCharacterFrame" )
            hooksecurefunc("CharacterFrame_ShowSubFrame", function(frameName)
                local show = (frameName == "MyRolePlayCharacterFrame");
                MyRolePlayCharacterFrame:SetShown(show);
            end);

            -- Increase the number of tabs to match (Taints CharacterFrame.numTabs as of 8.2)
            ...
            hooksecurefunc("PanelTemplates_UpdateTabs", function(frame)
                ...
                elseif frame.selectedTab == (mrp.oldNumTabs + 1) then
            ...
            -- Fix issues with the appropriate icon not showing when the UI
            -- is initially opened sometimes.
            hooksecurefunc("CharacterFrame_UpdatePortrait", function()
                if MyRolePlayCharacterFrame:IsShown() then
