...
mrp.DisplayBrowser = setmetatable( {
    ...
    ["NT"] = function( contents )
        local t = mrp.Display.NT( contents )
        if t and #t > 70 then
            t = clean( strsub( t, 1, 67 ) ) .. "…"
        end
        return t or ""
    end,
}, {
...
mrp.DisplayTooltip = setmetatable( {
    ["VA"] = function( contents )
        if emptynil( contents ) then
            ...
            if(mrpSaved.Options.ShowFullVersionText == false) then
                if strfind( contents, "MyRolePlay/", 1, true ) then
                    tinsert(s, "MyRolePlay")
                end
                ...
                if strfind( contents, "Vernacular/", 1, true ) then
                    tinsert(s, "Vernacular")
                end
            else
