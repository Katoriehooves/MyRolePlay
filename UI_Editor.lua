...
-- FIXME: Classic merge hacks.
local EditorPortraitTextureWidget;
...
local function mrp_MusicCallback(selectedMusic)
...
local function mrp_IconCallback(selectedTexture)
    local profile = mrpSaved.SelectedProfile
    local fullName = select(1, UnitFullName("player")) .. "-" .. select(2, UnitFullName("player")) -- Doing this way seems less risky than running mrp:SetCurrentProfile(), even if it'd be easier.
    if(selectedTexture) then
        mrp:SaveField( "IC", selectedTexture )
        SetPortraitToTexture( EditorPortraitTextureWidget, "Interface\\ICONS\\" .. mrpSaved.Profiles[mrpSaved.SelectedProfile]["IC"] )
    else
        mrpSaved.Profiles[profile]["IC"] = nil
        msp.char[fullName]["field"]["IC"] = nil
        msp.my["IC"] = nil
        SetPortraitTexture( EditorPortraitTextureWidget, "player" )
    end
end

--

local function MyRolePlayCharacterFrame_Profile_Dropdown_Click( self, profile )
...
local function MyRolePlayCharacterFrame_Profile_Dropdown_Init( self )
...
function mrp:CreateCharacterFrame()
    if not MyRolePlayCharacterFrame then
        ...
        if mrp:IsMainlineClient() then
            cf = CreateFrame( "Frame", "MyRolePlayCharacterFrame", CharacterFrame, nil, 5)
            EditorPortraitTextureWidget = CharacterFramePortrait;
        elseif mrp:IsClassicClient() then
            cf = CreateFrame( "Frame", "MyRolePlayCharacterFrame", CharacterFrame, "PortraitFrameTemplate", 5)
            EditorPortraitTextureWidget = MyRolePlayCharacterFramePortrait;
        ...
        cf:SetScript("OnShow", function(self)
            ...
            EditorPortraitTextureWidget:SetTexCoord( 0, 1, 0, 1 )
        ...
        cf:SetScript("OnHide", function(self)
            ...
            elseif mrp:IsClassicClient() then
                ...
                CharacterNameText:SetText(origCharacterNameTitle)
            end
        end	)
        ...
        if mrp:IsMainlineClient() then
            ...
            cf.pcb:SetPoint( "TOP", CharacterFrameInset, "TOP", 60, 18 )
        elseif mrp:IsClassicClient() then
            ...
            -- Version
            cf.ver = cf:CreateFontString( nil, "OVERLAY", "GameFontNormalSmall" )
            cf.ver:SetPoint( "TOPLEFT", MyRolePlayCharacterFrame, "TOPLEFT", 103, -27 )
            cf.ver:SetText( mrp.VerText )
        ...
        -- [I]
        cf.icb = CreateFrame( "Button", "MyRolePlayCharacterFrame_IconButton", cf, "UIPanelButtonTemplate" )
        ...
        -- [Settings button]
        cf.cfg = CreateFrame( "Button", "MyRolePlayCharacterFrame_SettingsButton", cf, "UIPanelButtonTemplate" )
		if mrp:IsMainlineClient() then
			cf.cfg:SetPoint( "BOTTOM", cf.icb, "TOP", -1, -1 )
		elseif mrp:IsClassicClient() then
			cf.cfg:SetPoint( "BOTTOM", cf.icb, "TOP", -10, -1 )
		end
		...
		-- [Change Log button]
        cf.clb = CreateFrame( "Button", "MyRolePlayCharacterFrame_ChangeLogButton", cf, "UIPanelButtonTemplate" )
        cf.clb:SetPoint( "LEFT", cf.cfg, "RIGHT", -2, 0 )
        cf.clb:SetText( L["|TInterface\\Buttons\\UI-GuildButton-PublicNote-Up:10:11:0:-1|t"] )
        cf.clb:SetWidth( 18 )
        cf.clb:SetHeight( 16 )
        cf.clb:SetScript( "OnEnter", function(self)
            GameTooltip:SetOwner( self, "ANCHOR_RIGHT" )
            GameTooltip:SetText( L["editor_changelog_button"], 1.0, 1.0, 1.0 )
            GameTooltip:AddLine( L["editor_changelog_button_tt"], 1.0, 0.8, 0.06, true)
            GameTooltip:Show()
        end )
        cf.clb:SetScript( "OnLeave", GameTooltip_Hide )
        cf.clb:SetScript("OnClick", function (self)
            mrp:FormatChangeLog() -- Setup and show changelog for a new version.
            MyRolePlayChangeLogFrame:Show();
        end )

        -- [Bug Report button]
        cf.brb = CreateFrame( "Button", "MyRolePlayCharacterFrame_BugReportButton", cf, "UIPanelButtonTemplate" )
        cf.brb:SetPoint( "LEFT", cf.clb, "RIGHT", -2, 0 )
        cf.brb:SetText( L["|TInterface\\HELPFRAME\\HelpIcon-Bug:18:18:0:-2|t"] )
        cf.brb:SetWidth( 18 )
        cf.brb:SetHeight( 16 )
        cf.brb:SetScript( "OnEnter", function(self)
            GameTooltip:SetOwner( self, "ANCHOR_RIGHT" )
            GameTooltip:SetText( L["editor_reportbug_button"], 1.0, 1.0, 1.0 )
            GameTooltip:AddLine( L["editor_reportbug_button_tt"], 1.0, 0.8, 0.06, true)
            GameTooltip:Show()
        end )
        cf.brb:SetScript( "OnLeave", GameTooltip_Hide )
        cf.brb:SetScript("OnClick", function (self)
            if mrp:IsMainlineClient() then
                mrp:ShowHyperlinkBox("https://www.curseforge.com/wow/addons/my-role-play/issues", "https://www.curseforge.com/wow/addons/my-role-play/issues");
            elseif mrp:IsClassicClient() then
                mrp:ShowHyperlinkBox("https://www.curseforge.com/wow/addons/myroleplay-classic/issues", "https://www.curseforge.com/wow/addons/myroleplay-classic/issues");
            end
        end )

        -- [M]
        cf.msb = CreateFrame( "Button", "MyRolePlayCharacterFrame_IconButton", cf, "UIPanelButtonTemplate" )
        ...
        cf.msb:SetScript( "OnEnter", function(self)
            ...
            if(mrpSaved.Profiles[mrpSaved.SelectedProfile]["MU"] and mrpSaved.Profiles[mrpSaved.SelectedProfile]["MU"] ~= "") then
                ...
                local musicName
                local musicFileID = tonumber(mrpSaved.Profiles[mrpSaved.SelectedProfile]["MU"])
                if musicFileID  then -- Convert ID to a name if it looks like a number.
                    musicName = LibRPMedia:GetMusicNameByFile(musicFileID)
                else
                    musicName = mrpSaved.Profiles[mrpSaved.SelectedProfile]["MU"]
                end
                GameTooltip:AddLine( musicName, 1.0, 0.8, 0.06)
                GameTooltip:Show()
            else
        ...
        -- [+]
        cf.npb = CreateFrame( "Button", "MyRolePlayCharacterFrame_NewProfileButton", cf, "UIPanelButtonTemplate" )
        ...
        -- [R]
        cf.rpb = CreateFrame( "Button", "MyRolePlayCharacterFrame_RenProfileButton", cf, "UIPanelButtonTemplate" )
        ...
        -- [-]
        cf.dpb = CreateFrame( "Button", "MyRolePlayCharacterFrame_DelProfileButton", cf, "UIPanelButtonTemplate" )
        ...
        -- [Import]
        cf.ipb = CreateFrame( "Button", "MyRolePlayCharacterFrame_ImportProfileButton", cf, "UIPanelButtonTemplate" )
        cf.ipb:SetPoint( "BOTTOMRIGHT", cf.dpb, "TOPRIGHT", 1, 0 )
        ...
        if mrp:IsMainlineClient() then
            cf.f:SetPoint( "TOPLEFT", CharacterFrameInset, "TOPLEFT", 6, -3 )
            cf.f:SetPoint( "TOPRIGHT", CharacterFrameInset, "TOPRIGHT", -5, -3 )
            cf.f:SetPoint( "BOTTOMLEFT", CharacterFrameInset, "BOTTOMLEFT", 6, 6 )
        ...
        mrp.CreateCharacterFrame = nop
        mrp.CreateCFpfield = nop
    end
end
    ...
    for index, field in pairs( cf.f.fields ) do
        if(index ~= "PE" and index ~= "PS") then -- We need to handle glances and traits differently.
            if(index == "DE" or index == "HI") then
                field.t:SetText( mrp.Display[ index ]( msp.my[ index ] ):gsub("{.-}", ""):gsub("|T.-|t", "") )
            else
        ...
        elseif(index == "PE") then
            if(mrpSaved.Profiles[mrpSaved.SelectedProfile]["glances"]) then
                local glanceIconRow = ""
                for i = 1, 5, 1 do
                    if(mrpSaved.Profiles[mrpSaved.SelectedProfile]["glances"][i] and mrpSaved.Profiles[mrpSaved.SelectedProfile]["glances"][i]["Icon"] and mrpSaved.Profiles[mrpSaved.SelectedProfile]["glances"][i]["Icon"] ~= "Interface\\Icons\\INV_Misc_QuestionMark") then
